...
if __name__ == '__main__':
    ...
    counter_org = Counter(PM21_y)
    for k_org, v1_org in counter_org.items():
        per_org = v1_org / len(PM21_y) * 100
        print('Class=%d, n=%d (%.3f%%)' % (k_org, v1_org, per_org))

    #df_test_droped = drop_null(df_test)
    ...
    # print(" === test data === ")
    # print("null num :", df_test_droped.isnull().sum().sum())
    # print(df_test_droped.shape)

    # df_testing = shuffle(df_test_droped)
    
    # tt_X = df_testing.iloc[1:2000, 3:-1]
    # tt_y = df_testing.iloc[1:2000, -1]
    ...
    X_train_scale, X_test_scale = Method_Transforms.Gernal_Scaler()
    
    
    # train 
    x_train_df = pd.DataFrame(X_train_scale.astype(float))
    y_train_df = pd.Series(y_train, )
    y_train_df.reset_index(drop=True, inplace=True)
    
    # test
    x_test_df = pd.DataFrame(X_test_scale.astype(float))
    ...
    def Distribution(counter):
        ...
        plt.xlabel("Class")
        plt.ylabel("Count")
        plt.bar(x, counter.values(), color=['green', 'darkred'])
        
    Distribution(counter_org)
    
    #X_train, X_test, y_train, y_test
    # Classification  X_train, X_test, y_train, y_test
    
    # label encoding
    CLF = CLF_Method(x_train_df, x_test_df , y_train_df, y_test_df, 12)
    ...
    # f1_true, f1_pred = tt_y.astype("float") , tt_X.astype("float")
    ...
    # plot_confuse_matrix(tt_y.astype("float"), Xgb_pred)
